...
import { preventSyntheticEvent } from '@utils/index'

const LOGO = require('@static/icon/logo.png')
...
import LUOO from '@static/icon/Luoo.qy.svg'
...
const CNIcon: SFC<IProps> = ({
  ...
  const handleClick = (e: React.FormEvent<HTMLImageElement>) => {
    if (preventDefault) preventSyntheticEvent(e)
    onClick && onClick(e)
  }
  return (
    <img
        ...
        onClick={handleClick}
        className={`icon-${type.toLocaleLowerCase()} ${className}`}
      />
  )
}
